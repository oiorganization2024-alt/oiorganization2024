...
def get_payment_success_email(name, amount, full_date, month_name, total_savings):
    return f"{t('প্রিয়', 'Dear')} {name},\n\n✅ {t('পেমেন্ট সফল', 'Payment Success')}\n{t('তারিখ', 'Date')}: {full_date}\n{t('মাস', 'Month')}: {month_name}\n{t('পরিমাণ', 'Amount')}: {amount} {t('টাকা', 'Taka')}\n{t('মোট', 'Total')}: {total_savings} {t('টাকা', 'Taka')}"
...
def get_withdrawal_notification(name, amount, description, date, previous_balance, current_balance):
    return f"{t('প্রিয়', 'Dear')} {name},\n\n🏧 {t('টাকা উত্তোলন', 'Withdrawal')}\n{t('তারিখ', 'Date')}: {date}\n{t('পরিমাণ', 'Amount')}: {amount}\n{t('বিবরণ', 'Description')}: {description}"

def get_lottery_winner_email(name):
    return f"{t('প্রিয়', 'Dear')} {name},\n\n🎉 {t('অভিনন্দন', 'Congratulations')}! {t('লাকি ড্র বিজয়ী', 'Lucky draw winner')}!"
...
def get_member_transactions(member_id):
    try:
        ...
        c = conn.cursor()
        c.execute("""
            SELECT id, full_date, amount, month_name, year, late_fee, note
            FROM transactions 
            WHERE member_id = ?
...
def generate_pdf_transactions(member_id=None):
    ...
    if member_id:
        ...
        for t in trans:
            data.append([t[1], f"{t[2]:,.0f}", t[3], str(t[4])])
    else:
        data = [[t('তারিখ', 'Date'), t('পরিমাণ', 'Amount')]]
    if data:
        table = Table(data)
        table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1a5276')),
...
def member_login_page(member_id):
    ...
    if not member:
        ...
        return
    st.title(f"{SOMITI_NAME} - Member Login")
    st.markdown(f"### Welcome, {member[1]}")
...
def member_dashboard_view():
    ...
    st.title(f"{SOMITI_NAME} - Member Dashboard")
    st.markdown(f"### Welcome, {name}!")
    st.metric("Total Savings", f"{total_savings:,.0f} Taka")
    with st.sidebar:
        if st.button("Logout"):
...
def admin_login_page():
    apply_dark_theme()
    st.title(f"{SOMITI_NAME} - Admin Login")
    ...
    if st.button("Login", type="primary"):
        if phone == ADMIN_MOBILE and password == ADMIN_PASSWORD:
            ...
            st.error("Wrong mobile or password")
...
def admin_panel():
    ...
    st.title(f"🌾 {SOMITI_NAME} 🌾")
    col1, col2 = st.columns(2)
    col1.metric(f"💰 {t('মোট জমা', 'Total Savings')}", f"{total:,.0f} {t('টাকা', 'Taka')}")
    col2.metric(f"💵 {t('ক্যাশ ব্যালেন্স', 'Cash Balance')}", f"{cash:,.0f} {t('টাকা', 'Taka')}")
    ...
    with st.sidebar:
        ...
        menu = st.radio(t("নির্বাচন করুন", "Select"), menu_options, label_visibility="collapsed")
    ...
    # ড্যাশবোর্ড
        ...
        with col1:
            if st.button("📢 ১ তারিখ রিমাইন্ডার", use_container_width=True, type="primary"):
                sent = send_email_to_all_active_members(
                    f"📢 {t('মাসিক কিস্তি রিমাইন্ডার', 'Monthly Reminder')} - {SOMITI_NAME}",
                    f"{t('প্রিয়', 'Dear')} {{name}},\n\n{t('আগামী ১০ তারিখের মধ্যে কিস্তি পরিশোধ করুন', 'Please pay by 10th')}।"
                )
        ...
        with col2:
            if st.button("⚠️ ১০ তারিখ রিমাইন্ডার", use_container_width=True, type="primary"):
                ...
                for m in unpaid:
                    if m[5]:
                        msg = f"{t('প্রিয়', 'Dear')} {m[1]},\n\n{t('আপনার বকেয়া', 'Your due')}: {m[3]:,.0f} {t('টাকা', 'Taka')}\n{t('অনুগ্রহ করে পরিশোধ করুন', 'Please pay')}।"
    ...
    # টাকা জমা
        ...
        with tab1:
            ...
            if paid:
                for m in paid:
                    st.write(f"✅ {m[1]} ({m[0]}) - {m[4]:,.0f} {t('টাকা', 'Taka')}")
            else:
                        ...
                        if st.button(f"✅ {t('জমা নিন', 'Deposit')}", key=f"dep_{m[0]}", type="primary"):
                            ...
                            if m[5]:
                                send_email(m[5], f"✅ {t('পেমেন্ট সফল', 'Payment Success')} - {SOMITI_NAME}", 
                                          get_payment_success_email(m[1], f"{total:,.0f}", full_date, BANGLA_MONTHS[month], f"{new_total:,.0f}"))
                        ...
                        for t in trans:
                            ...
                            if c5.button("🗑️", key=f"del_{t[0]}"):
                                ...
                                if member[3]:
                                    send_email(member[3], f"🗑️ {t('লেনদেন বাতিল', 'Transaction Cancelled')}", 
                                              get_transaction_remove_email(member[1], f"{t[2]:,.0f}", t[1], f"{new_total:,.0f}"))
                            ...
                            if st.session_state.get(f"edit_trans_{t[0]}"):
                                with st.form(f"edit_{t[0]}"):
                                    new_amt = st.number_input(t("টাকা", "Amount"), value=float(t[2]), step=50.0)
                                    if st.form_submit_button(f"💾 {t('সেভ', 'Save')}", type="primary"):
                                        ...
                                        if member[3]:
                                            send_email(member[3], f"✏️ {t('লেনদেন সংশোধন', 'Transaction Edit')}", 
                                                      get_transaction_edit_email(member[1], f"{t[2]:,.0f}", f"{new_amt:,.0f}", t[1], f"{new_total:,.0f}"))
    ...
    # সদস্য লিংক
        ...
        for m in members:
            ...
            st.write(f"**{name} ({member_id})**")
            st.write(f"📱 {phone} | 📧 {email or 'N/A'}")
            st.code(link)
            st.write(f"🔑 {password}")
            
            c1, c2, c3 = st.columns(3)
            with c1:
                if st.button(f"📋 {t('লিংক কপি', 'Copy Link')}", key=f"copy_link_{member_id}"):
                    st.code(link)
            with c2:
                if st.button(f"📋 {t('পাসওয়ার্ড কপি', 'Copy Pass')}", key=f"copy_pass_{member_id}"):
                    st.code(password)
            with c3:
            ...
            if st.form_submit_button(f"✅ {t('উত্তোলন করুন', 'Withdraw')}", type="primary"):
                if amount > 0 and amount <= cash:
                    if description:
                        ...
                        sent = send_email_to_all_active_members(
                            f"🏧 {t('টাকা উত্তোলনের নোটিশ', 'Withdrawal Notice')} - {SOMITI_NAME}",
                            get_withdrawal_notification("{name}", f"{amount:,.0f}", description, date.strftime('%d %B %Y'), f"{cash:,.0f}", f"{cash - amount:,.0f}")
                        )
        ...
        with tab2:
            ...
            if unpaid:
                ...
                if st.button(f"📧 {t('বকেয়া রিমাইন্ডার', 'Due Reminder')}", type="primary"):
                    ...
                    for m in unpaid:
                        if m[5]:
                            msg = f"{t('প্রিয়', 'Dear')} {m[1]},\n\n{BANGLA_MONTHS[datetime.now().month]} {t('মাসের কিস্তি', 'installment')} ({m[3]:,.0f} {t('টাকা', 'Taka')}) {t('বকেয়া আছে', 'is due')}।"
                            if send_email(m[5], f"⚠️ {t('বকেয়া রিমাইন্ডার', 'Due Reminder')}", msg):
                                sent += 1
                    st.success(f"✅ {sent} {t('জনকে পাঠানো হয়েছে', 'sent')}!")
